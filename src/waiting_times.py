...
def plot_waiting_time_histogram(waiting_times, log_scale=False, n_flips=None, save_path=None):
    """绘制等待时间直方图并保存图片"""
    ...
    # 保存图片或显示
    if save_path:
        filename = f"{title.replace(':', '').replace(' ', '_')}.png"
        plt.savefig(f"{save_path}/{filename}")
        print(f"图像已保存到: {save_path}/{filename}")
    else:
        plt.show()
...
def run_experiment(n_flips, title, save_path=None):
    """运行一次等待时间实验
    ...
    # 绘制直方图并保存
    plot_waiting_time_histogram(waiting_times, log_scale=False, n_flips=n_flips, save_path=save_path)
    plot_waiting_time_histogram(waiting_times, log_scale=True, n_flips=n_flips, save_path=save_path)
...
if __name__ == "__main__":
    ...
    # 指定保存路径
    save_path = "C:/Users/31025/OneDrive/桌面/t"

    # 任务一：1000次抛掷
    waiting_times_1k, stats_1k = run_experiment(1000, "Task 1: 1000 Coin Flips", save_path=save_path)

    # 任务二：1000000次抛掷
    print("\n")
    waiting_times_1m, stats_1m = run_experiment(1000000, "Task 2: 1,000,000 Coin Flips", save_path=save_path)
